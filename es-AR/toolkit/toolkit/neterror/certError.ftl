...
cert-error-mitm-connection = { -brand-short-name } usa el almacén de BrowserWorks de CA para verificar que la conexión sea segura, en lugar de los certificados proporcionados por el sistema operativo del usuario. Por lo tanto, si un programa antivirus o una red está interceptando una conexión con un certificado de seguridad emitido por una CA que no está en el almacén de CA de BrowserWorks, la conexión se considera insegura.
...
fp-certerror-revoked-why-dangerous-body = { -brand-short-name } está advirtiendo sobre este sitio porque el certificado provisto para { $hostname } fue revocado y ya no es confiable.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
fp-certerror-revoked-what-can-you-do-body = Probablemente nada, ya que parece que hay un problema con el sitio. Puede consultar con el propietario del sitio web para ver si está trabajando en el problema.
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
...
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-certerror-untrusted-issuer-why-dangerous-body = { -brand-short-name } está avisando sobre este sitio porque el certificado proporcionado para { $hostname } fue emitido por una autoridad de certificación que ya no es de confianza.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
fp-certerror-untrusted-issuer-what-can-you-do-body = Probablemente nada, ya que parece que hay un problema con el sitio. Puede consultar con el propietario del sitio web para ver si está trabajando en el problema.
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
...
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-certerror-invalid-cert-why-dangerous = El propietario de { $hostname } no lo configuró correctamente y no se pudo crear una conexión segura.
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-intro = Su computadora cree que hoy es { DATETIME($now, dateStyle: "medium") }, lo que impide que { -brand-short-name } se conecte de forma segura. Para visitar <strong>{ $hostname }</strong>, actualice la hora de la computadora en los ajustes del sistema a la hora, fecha y zona horaria actuales, y luego recargue la página.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
# Variables:
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-what-can-you-do-body = Actualice el reloj de su computadora en la configuración del sistema a la hora, fecha y huso horario actuales. La computadora cree que es { DATETIME($now, dateStyle: "medium") }.
...
fp-certerror-what-can-you-do = ¿Qué se puede hacer al respecto?
...
fp-certerror-override-exception-button-2 = Continuar a { $hostname } (Riesgoso)
    .accesskey = g
