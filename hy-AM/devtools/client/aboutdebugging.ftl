...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Միացրեք USB վրիպազերծումը Android սարքի Waterfox-ում։
# USB section step by step guide
...
about-debugging-browser-version-too-recent = Միացված դիտարկիչը ավելի նոր է ({ $runtimeVersion }, buildID { $runtimeID }) քան ձեր { -brand-shorter-name } ({ $localVersion }, buildID { $localID })։ Սա չաջակցված կարգավորում է և կարող է հանգեցնել DevTools-ին չստացվել։ Խնդրում ենք թարմացրել Waterfox-ը։ <a>Անսարքությունների կարգավորում</a>
