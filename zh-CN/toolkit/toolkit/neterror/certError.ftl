...
cert-error-mitm-connection = { -brand-short-name } 使用 BrowserWorks 的数字证书认证机构存储库来验证连接是否安全，而非用户操作系统所提供的证书库。因此，如果您的防病毒软件或网络使用不在 BrowserWorks 数字证书认证机构列表中的机构所颁发的证书来拦截网络流量，该连接被视为不安全。
