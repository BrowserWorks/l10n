...
fp-certerror-revoked-why-dangerous-body = 由於 { $hostname } 提供的憑證已遭廢止，不再受信任，所以 { -brand-short-name } 才會警告您。
...
fp-certerror-clock-skew-intro = 您的電腦認為目前時間為 { DATETIME($now, dateStyle: "medium") }，所以 { -brand-short-name } 無法建立安全連線。若要造訪 <strong>{ $hostname }</strong>，請到系統設定中確認日期、時間、時區設定是否正確，然後重新載入頁面。
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
# Variables:
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-what-can-you-do-body = 請到系統設定中調整日期、時間、時區設定。您的電腦認為現在是 { DATETIME($now, dateStyle: "medium") }。

## Variables:
##   $hostname (string) - Hostname of the website with cert error.

cert-error-revoked-certificate = 由於簽發給 { $hostname } 的憑證已遭廢止，不再受信任，{ -brand-short-name } 封鎖了您對此網站的造訪。
