...
addon-install-error-hard-blocked = { $addonName } so přećiwo prawidłam BrowserWorks přeńdźe a njeda so na { -brand-short-name } instalować.
...
addon-install-error-soft-blocked = { $addonName } so přećiwo prawidłam BrowserWorks přeńdźe a njeda so na { -brand-short-name } instalować.
