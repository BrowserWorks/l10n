...
cert-error-mitm-mozilla = { -brand-short-name } is supportit by the non-profit BrowserWorks, that admeenisters a halely open certificate authority (CA) store. The CA stores helps mak shair that certificate authorities are follaein best practices fur yaiser siccarness.
cert-error-mitm-connection = { -brand-short-name } yaises the BrowserWorks CA store fur tae mak shair that a connection is siccar, raither than certificates providit by the yaiser's operatin system. Sae, gin an antivirus program or a netwark is interceptin a connection wi a siccarness certificate issued by a CA that isnae in the BrowserWorks CA store, the connection is conseedert no sauf.
