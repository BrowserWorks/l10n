...
cert-error-mitm-mozilla = { -brand-short-name } es sostengut per BrowserWorks, una organizacion sens but lucratiu que gerís un magasin d’autoritats de certificacion (CA) complètament dobèrt. Lo magasin ajuda a assegurar que las autoritats de certificacion respècten las melhoras practicas de seguretat per protegir los utilizaires.
cert-error-mitm-connection = { -brand-short-name } utiliza lo magasin d’autoritats de certificacion de BrowserWorks per verificar qu’una connexion es segura, allòc d’utilizar los certificats del sistèma operatiu de l’utilizaire., Atal, s’un programa anti-virus o un ret intercèptan una connexion amb un certificat de seguretat emés per una CA que se tròba pas al magasin de BrowserWorks, la connexion es considera coma pas segura.
