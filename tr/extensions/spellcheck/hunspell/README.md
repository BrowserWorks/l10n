# Hunspell Turkish Dictionary for Waterfox

A highly accurate Turkish spellchecking dictionary (`.aff` / `.dic`) optimized, compressed, and packaged specifically for use in **Waterfox** as a browser extension.

This project packages state-of-the-art Turkish dictionary data into a legacy/WebExtension format (`.xpi`) so it can be directly loaded into Waterfox for seamless, native browser spellchecking.
...
*   **File Size Reduction:** Stripped redundant word stems, duplicated flag combinations, and obsolete word definitions. This drastically reduces memory overhead when Waterfox loads the dictionary into RAM.
...
## 🚀 How to Install in Waterfox

Because this is optimized specifically as a Waterfox extension, you can load it directly into your browser:

1. Open **Waterfox**.
2. Type `about:debugging` in the URL address bar and press **Enter**.
3. Click on **"This Waterfox"** in the left sidebar.
