...
cert-error-mitm-mozilla = El { -brand-short-name } té el suport de l'organització sense ànim de lucre BrowserWorks, que gestiona un magatzem d'entitat certificadora (CA) completament obert. El magatzem de CA vos garanteix que les entitats certificadores segueixen les millors pràctiques per a la seguretat dels usuaris.
cert-error-mitm-connection = El { -brand-short-name } utilitza el magatzem de CA de BrowserWorks per verificar que una connexió és segura, en lloc d'utilitzar certificats subministrats pel sistema operatiu de l'usuari. Per tant, si un programa antivirus o una xarxa intercepten una connexió amb un certificat de seguretat emés per una CA que no es troba al magatzem de CA de BrowserWorks, la connexió es considera insegura.
