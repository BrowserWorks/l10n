...
cert-error-mitm-mozilla = { -brand-short-name } se wót za wše wužytneje załožby BrowserWorks pódpěra, kótaraž dopołnje wótwórjony wobchod certificěrowanišća (CA) zarědujo. Wobchod certificěrowanišća pomaga zawěsćiś, až se certificěrowanišća za nejlěpšymi praktikami za wužywaŕsku wěstotu źarže.
cert-error-mitm-connection = { -brand-short-name } wobchod certificěrowanišćow BrowserWorks wužywa, aby pśeglědował, lěc zwisk jo wěsty, a nic certifikaty ze źěłowego systema wužywarja. Jolic pótakem antiwirusowy program abo seś zwisk z wěstotnym certifikatom wótpópadnjo, kótarež jo certificěrowanišćo wudało, kótarež njejo we wobchoźe certificěrowanišćow BrowserWorks , se zwisk ma za njewěsty.
...
fp-certerror-revoked-what-can-you-do-body = Nejskerjej nic, dokulaž to jo nejskerjej problem ze sedłom samym. Móžośo se pla wobsejźarja websedła wobgóniś, aby zgónił, lěc na problemje źěła.
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
...
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-certerror-transparency-why-dangerous-body = { -brand-short-name } { $hostname } njedowěrja, dokulaž njejo mógał dopokazaś, až pominanja transparence za zjawne certifikaty docynja.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
fp-certerror-transparency-what-can-you-do-body = Nejskerjej nic, dokulaž jo wěrjepódobnje, až problem jo ze sedłom samym.
...
# Variables:
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-what-can-you-do-body = Aktualizěrujśo zeger swójogo licadła w swójich systemowych nastajenjach na aktualny datum, cas a casowe pasmo. Wašo licadło mysli, až jo { DATETIME($now, dateStyle: "medium") }.
