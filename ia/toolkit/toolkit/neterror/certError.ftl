...
cert-error-mitm-mozilla = { -brand-short-name } es sustenite per le organisation sin scopo lucrative BrowserWorks, que administra un magazin de autoritates de certification (CA) completemente aperte. Le magazin CA adjuta a garantir que le autoritates de certification seque le melior practicas pro le securitate del usator.
cert-error-mitm-connection = { -brand-short-name } usa le magazin CA de BrowserWorks pro verificar que un connexion es secur, in vice de usar le certificatos fornite per le systema de operation del usator. Dunque, si un programma antivirus o un rete intercepta le connexion con un certificato de securitate emittite per un CA que non es in le magazin CA de BrowserWorks, le connexion es considerate insecur.
...
fp-certerror-revoked-why-dangerous-body = { -brand-short-name } te avisa re iste sito perque le certificato fornite pro { $hostname } ha essite revocate e non plus es de confidentia.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
fp-certerror-revoked-what-can-you-do-body = Probabilemente nihil, pois que il es probabile que il ha un problema con le sito mesme. Tu pote verificar con le proprietario del sito web pro vider si ille sta a labora sur le problema.
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
...
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-certerror-untrusted-issuer-why-dangerous-body = { -brand-short-name } te avisa re iste sito perque le certificato fornite pro { $hostname } era publicate per un autoritate de certification que non plus es de confidentia.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
...
fp-certerror-pkix-not-yet-valid-why-dangerous-body = { -brand-short-name }non se fide de iste sito, perque il pare que le certificato fornite non es valide ante le { DATETIME($date, month: "numeric", day: "numeric", year: "numeric") } de { DATETIME($date, timeStyle: "short") }
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
# Variables:
#   $date (Date) - Device's clock date.
fp-certerror-pkix-not-yet-valid-what-can-you-do-body = Le horologio de tu apparato es configurate a { DATETIME($date, timeStyle: "short") } { DATETIME($date, month: "numeric", day: "numeric", year: "numeric") }. Si isto es correcte, le problema de securitate es probabilemente con le sito mesme. Si illo es errate, tu pote cambiar illo in le parametros de systema de tu apparato.
# This string appears after the following string: "What makes the site look dangerous?" (fp-certerror-why-site-dangerous)
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-certerror-invalid-cert-why-dangerous = Le proprietario de { $hostname } non ha configurate le sito correctemente e es impossibile crear un connexion secur.
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-intro = Tu computator pensa que es le { DATETIME($now, dateStyle: "medium") }, lo que impedi a { -brand-short-name } de connecter se con securitate. Pro visitar <strong>{ $hostname }</strong>, actualisa le horologio de tu computator in le parametros de tu systema al data, tempore, e fuso horari currente, e pois actualisa le pagina.
# This string appears after the following string: "What can you do about it?" (fp-certerror-what-can-you-do)
# Variables:
#   $now (Date) - The current datetime, to be formatted as a date
fp-certerror-clock-skew-what-can-you-do-body = Actualisa le horologio de tu computator in le parametros de tu systema al data, tempore, e fuso horari currente. Tu computator pensa que il es le { DATETIME($now, dateStyle: "medium") }.
