...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Povoľte ladenie cez USB v prehliadači Waterfox na zariadení s Androidom.
# USB section step by step guide
...
about-debugging-browser-version-too-recent = Pripojený prehliadač je novší ({ $runtimeVersion }, buildID { $runtimeID }) ako váš { -brand-shorter-name } ({ $localVersion }, buildID { $localID }). Toto je nepodporovaná konfigurácia a môže spôsobiť, že ladenie prestane fungovať. Aktualizujte prosím svoj Waterfox. <a>Riešenie problémov</a>
