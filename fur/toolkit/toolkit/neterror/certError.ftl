...
cert-error-mitm-mozilla = { -brand-short-name } al è supuartât de organizazion cence fins di vuadagn BrowserWorks, che e aministre un archivi di autoritâts di certificazion (CA) dal dut viert. L'archivi CA al jude a garantî che lis autoritâts di certificazion a ledin daûr des miôrs pratichis  pe sigurece dal utent.
cert-error-mitm-connection = { -brand-short-name } al dopre l'archivi CA di BrowserWorks par verificâ che une conession e sedi sigure, invezit che i certificâts furnîts dal sisteme operatîf dal utent. Cussì, se un program antivirus o une rêt e sta intercetant une conession cuntun certificât di sigurece mandât fûr di une CA che no je tal archivi CA di BrowserWorks, la conession e ven considerade no sigure.
