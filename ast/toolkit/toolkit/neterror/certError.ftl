...
cert-error-mitm-mozilla = { -brand-short-name } ta sofitáu pola organización ensin ánimu de llucru BrowserWorks, que xestiona un almacén d'entidá certificadora (CA) abiertu dafechu. L'almacén CA ayúdanos a asegurar que les entidaes certificadores siguen les meyores práutiques pa la seguranza de los usuarios.
cert-error-mitm-connection = { -brand-short-name } usa l'almacén CA de BrowserWorks pa verificar qu'una conexón ye segura, en cuentes d'usar los certificaos forníos pol sistema operativu del usuariu. Polo tanto, si un antivirus o una rede interceuten una conexón con un certificáu de seguranza emitíu por una CA que nun tan nel almacén CA de BrowserWorks, la conexón considérase insegura.
